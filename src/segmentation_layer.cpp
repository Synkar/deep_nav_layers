...
void SegmentationLayer::onInitialize()
{
  ...
  std::cout << "reading file " << homography_folder + "/homography.xml" << std::endl;
...
{
  cv::FileStorage fs = cv::FileStorage(homography_folder + "/segmentation_layer_parameters.yaml", cv::FileStorage::READ);
...
void SegmentationLayer::updateCosts(costmap_2d::Costmap2D& master_grid, 
                                    int min_i, int min_j, 
                                    int max_i, int max_j)
{
  if (!enabled_)
